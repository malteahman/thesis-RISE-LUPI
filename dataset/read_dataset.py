...
class DatasetClass(Dataset):
    def __init__(self, config, part):
        ...
        with open('/raid/dlgroupmsc/copy_and_paste/instances_4.txt', 'r') as file:
            self.instances_4 = json.load(file)
            self.instances_4 = [(list(d.keys())[0], list(d.values())[0]) for d in self.instances_4]
            num_of_list = int( len(self.instances_4) * config.frac_copy_samples)
            self.instances_4 = random.sample(self.instances_4, num_of_list)
        with open('/raid/dlgroupmsc/copy_and_paste/instances_9.txt', 'r') as file:
            self.instances_9 = json.load(file)
            self.instances_9 = [(list(d.keys())[0], list(d.values())[0]) for d in self.instances_9]
            num_of_list = int( len(self.instances_9) * config.frac_copy_samples)
            self.instances_9 = random.sample(self.instances_9, num_of_list)
        with open('/raid/dlgroupmsc/copy_and_paste/instances_12.txt', 'r') as file:
            self.instances_12 = json.load(file)
            self.instances_12 = [(list(d.keys())[0], list(d.values())[0]) for d in self.instances_12]
            num_of_list = int( len(self.instances_12) * config.frac_copy_samples)
            self.instances_12 = random.sample(self.instances_12, num_of_list)
    ...
    def __getitem__(self, index):
        ...
        if self.part == 'val' or self.part == 'test':
            x = self._normalize(x)
            if self.config.dataset.label_mask:
                x = np.concatenate((x,np.zeros((12, 512, 512))), axis=0)
        ...
        if self.config.dataset.label_mask:
            label_masks = self._label_mask(y)
            x = np.concatenate((x, label_masks), axis=0)

        copy_and_paste = self._do_copy_and_paste()
        if copy_and_paste != 'nan':
            x,y = self._copy_and_paste(x,y,copy_and_paste)
    ...
    def _do_copy_and_paste(self):
        if random.random() < self.config.copy_and_paste:
            value = random.choice([4,9,12])
            return value
        return 'nan'

    def _copy_and_paste(self,x,y,class_index):

        if class_index == 4:
            index = random.randint(0,len(self.instances_4)-1) 
            instances_c = self.instances_4
        elif class_index == 9:
            index = random.randint(0,len(self.instances_9)-1)
            instances_c = self.instances_9
        elif class_index == 12:
            index = random.randint(0,len(self.instances_12)-1)
            instances_c = self.instances_12

        mask_y_path, mask = instances_c[index]
        mask = torch.tensor(mask).unsqueeze(0)

        if torch.sum(mask) > 100*100: #take only 100*100 square
            max_index = 512 - 100
            start_row = random.randint(0, max_index)
            start_col = random.randint(0, max_index)
            new_mask = torch.zeros_like(mask)
            new_mask[start_row:start_row+100, start_col:start_col+100] = \
            mask[start_row:start_row+100, start_col:start_col+100]
            mask = new_mask

        mask = mask.repeat(self.config.model.n_channels,1,1)

        #print(torch.sum(mask).item()) #if sum is too big, take only fraction
        mask_x_path = mask_y_path.replace('labels', 'aerial').replace('msk', 'img').replace('MSK', 'IMG')
        copy_y = self._read_data(mask_y_path, True)
        copy_x = self._read_data(mask_x_path, False)
        copy_x = self._normalize(copy_x)
        copy_x, copy_y, _ = self.transform.apply(x,y,senti=None,channels=self.channels,use_senti=False)

        x[mask == 1] = copy_x[mask == 1]
        y[mask[0] == 1] = copy_y[mask[0] == 1]
    ...
    def _label_mask(self, y):
        label_mask = np.zeros((12, 512, 512))
        #sortera ut pixlarna från de 12 klasserna
        start_class = 0
        end_class = 11
        # Define the percentage of pixels to encode (1%)
        percentage_to_encode = 0.00005
        # Get the indices of all pixels where the class label falls within the range 0-12
        class_indices = np.where((y >= start_class) & (y <= end_class))
        # Calculate the total number of pixels
        total_pixels = class_indices[0].shape[0]
        # Calculate the number of pixels to encode (1% of total)
        num_pixels_to_encode = int(total_pixels * percentage_to_encode)
        # Randomly select 1% of pixels from all 12 classes combined
        random_indices = np.random.choice(total_pixels, size=num_pixels_to_encode, replace=False)
        # Extract the corresponding row and column indices from the class_indices
        selected_row_indices = class_indices[0][random_indices]
        selected_col_indices = class_indices[1][random_indices]
        # One-hot encode the selected pixels in label_mask
        for row_idx, col_idx in zip(selected_row_indices, selected_col_indices):
            class_label = y[row_idx, col_idx]
            label_mask[class_label, row_idx, col_idx] = 1
        return label_mask

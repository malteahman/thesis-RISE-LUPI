...
class DatasetPreparer:
    ...
    def prepare_dataset(self, part):
        ...
        if part == 'train' or part == 'val':
            ...
            senti_data_paths = senti_data_paths[:data_stop_point]
            senti_mask_paths = senti_mask_paths[:data_stop_point]
            senti_dates_paths= senti_dates_paths[:data_stop_point]

        # Save the paths to files
        with open(f'dataset/paths/X_paths_{part}_5_v2.txt', 'w') as f:
            for path in X_tif_paths:
                f.write("%s\n" % path)
        with open(f'dataset/paths/Y_paths_{part}_5_v2.txt', 'w') as f:
            for path in Y_tif_paths:
                f.write("%s\n" % path)
        with open(f'dataset/paths/senti_data_paths_{part}_5_v2.txt', 'w') as f:
            for path in senti_data_paths:
                f.write("%s\n" % path)
        with open(f'dataset/paths/senti_mask_paths_{part}_5_v2.txt', 'w') as f:
            for path in senti_mask_paths:
                f.write("%s\n" % path)
        with open(f'dataset/paths/senti_dates_paths_{part}_5_v2.txt', 'w') as f:

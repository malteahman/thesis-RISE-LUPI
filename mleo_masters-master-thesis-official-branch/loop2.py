...
def loop2(config, writer, hydra_log_dir):
    ...
    if config.model.name == 'teacher_student':
        teacher = get_teacher(config, config.model.teacher_student.teacher_path, config.model.teacher_student.teacher_channels, config.model.teacher_student.teacher_name)
    ...
    elif config.model.name == 'unet_predict_priv' or config.model.name == 'unet_reversed_predict_priv':
    ...
    while epoch < config.max_epochs:
        ...
        if True:
            for batch in tqdm(train_iter):
                ...
                with autocast():
                    if config.model.name == 'teacher_student':
                        model, y_pred, l = teacher_student(teacher, model, 'train', train_loss, x, y, mtd,
                                                        config.model.teacher_student.student_spec_channels,
                                                        config.model.teacher_student.teacher_spec_channels, config)
                    ...
                    elif config.model.name == 'unet_reversed_predict_priv':
                        y_pred, x_priv_pred = model(x) #2nd returns y_pred and prediction of RGB image, y is still truth
                        l,_,_,_,_ = train_loss(y_pred, x_priv_pred, y, x[:,:3,:,:])
                    elif config.model.name == 'unet_generate_priv':
                ...
                #     save_image(c, x_cpu, y_pred_cpu, y_cpu, epoch, config, writer)
                # c+=1
        ...
        if epoch % config.eval_every == 0:
            ...
            if config.model.name == teacher_student:
                teacher.eval()
            if config.model.name == 'unet_generate_priv' or config.model.name == 'unet_priv_forward' or config.model.name == 'unet_predict_priv' or config.model.name == 'unet_reversed_predict_priv':
                if config.model.name == 'unet_generate_priv':
                    priv_generator.eval()
            ...
            for batch in tqdm(val_iter):
                ...
                with torch.no_grad():
                    if config.model.name == 'teacher_student':
                        model, y_pred, l = teacher_student(teacher, model, 'val', eval_loss, x, y, mtd,
                                                        config.model.teacher_student.student_spec_channels,
                                                        config.model.teacher_student.teacher_spec_channels, config)
                    elif config.model.name == 'multi_teacher':
                ...
                if counter in idx_list and epoch % 15 == 0:
                    ...
                    if config.model.name == 'unet_predict_priv':
                        ...
                        priv_info_image(0,counter*10, x_priv_detached, epoch, writer)
                        priv_info_image(1,counter*10, x_priv_detached, epoch, writer)
                    if config.model.name == 'unet_reversed_predict_priv':
                        print(x_priv_pred.shape)
                        x_priv_detached = x_priv_pred[0,:,:,:].cpu().detach().contiguous().numpy() #RGB image
                        x_priv_detached = np.transpose(x_priv_detached, (1,2,0)).astype(float)
                        x_priv_detached *= np.array(config.dataset.std)[:3]
                        x_priv_detached += np.array(config.dataset.mean)[:3]
                        x_priv_detached = np.floor(x_priv_detached)
                        x_priv_detached = cv2.cvtColor(x_priv_detached.astype(np.uint8), cv2.COLOR_BGR2RGB) #convert from BGR to RGB
                        x_priv_detached = np.transpose(x_priv_detached, (2,0,1))
                        
                        x_priv_detached = x_priv_detached/255.0
                        x_priv_detached = torch.from_numpy(x_priv_detached)
                        writer.add_image('Epoch: ' + str(epoch) + ', Val/x, batch: ' + str(counter*10), x_priv_detached[:3], epoch)

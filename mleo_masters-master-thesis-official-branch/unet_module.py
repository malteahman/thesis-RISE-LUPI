...
class ReversedUnetPredictPriv(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(ReversedUnetPredictPriv, self).__init__()
        self.unet = smp.Unet(
            encoder_weights="imagenet",
            encoder_name="efficientnet-b4",
            in_channels = n_channels,
        ...
        self.B_decoder = smp.Unet(
            encoder_weights="imagenet",
            encoder_name="efficientnet-b4",
            in_channels = n_channels,
            classes=1 #one scalar per priv channel
        ).decoder
        self.B_segmentation_head = smp.Unet(
            encoder_weights="imagenet",
            encoder_name="efficientnet-b4",
            in_channels = n_channels,
            classes=1 #entire eight bit
        ).segmentation_head
        self.G_decoder = smp.Unet(
            encoder_weights="imagenet",
            encoder_name="efficientnet-b4",
            in_channels = n_channels,
            classes=1 #one scalar per priv channel
        ).decoder
        self.G_segmentation_head = smp.Unet(
            encoder_weights="imagenet",
            encoder_name="efficientnet-b4",
            in_channels = n_channels,
            classes=1 #entire eight bit
        ).segmentation_head
        self.R_decoder = smp.Unet(
            encoder_weights="imagenet",
            encoder_name="efficientnet-b4",
            in_channels = n_channels,
            classes=1 #one scalar per priv channel
        ).decoder
        self.R_segmentation_head = smp.Unet(
            encoder_weights="imagenet",
            encoder_name="efficientnet-b4",
            in_channels = n_channels,
            classes=1 #entire eight bit
    ...
    def forward(self, x):
        features = self.unet.encoder(x[:,3,:,:].unsqueeze(1))

        unet_decoded = self.unet.decoder(*features)
        y_pred = self.unet.segmentation_head(unet_decoded)

        B_decoded = self.B_decoder(*features)
        B_priv_pred = self.B_segmentation_head(B_decoded)

        G_decoded = self.G_decoder(*features)
        G_priv_pred = self.G_segmentation_head(G_decoded)

        R_decoded = self.R_decoder(*features)
        R_priv_pred = self.R_segmentation_head(R_decoded)

        RGB_priv_pred = torch.cat((B_priv_pred, G_priv_pred, R_priv_pred), dim=1)

        return y_pred, RGB_priv_pred

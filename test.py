...
def eval_on_test(config, writer, training_path):
    ...
    if config.eval.eval_type == "test_zero_out" or config.eval.eval_type  == 'test_zero_out_cf':
        with torch.no_grad():
    ...
    for batch in tqdm(test_iter):
        ...
        with torch.no_grad():
            ...
            if config.eval.eval_model == 'unet_predict_priv':
                y_pred, _ = model(x)
            elif config.eval.eval_model == 'resnet50':
                y_pred = model(x)["out"]
            else:
                y_pred = model(x)
    ...
    if config.eval.eval_type == 'test_cf' or config.eval.eval_type == 'test_zero_out_cf':
        conf_matrix(config, y_pred_list, y_list, writer, 0, only_first=True)
        writer.flush()
